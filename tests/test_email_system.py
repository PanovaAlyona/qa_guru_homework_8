...
from src.logging_service import LoggingEmailService
...
@pytest.mark.parametrize(
    "invalid", ["noatsymbol.com", "name@domain.xyz", "", "    "]
)
...
def test_add_short_body_boundary():
    email = Email(
        "s", "12345", EmailAddress("a@a.com"), EmailAddress("b@b.com")
    )
    email.add_short_body(5)
    assert email.short_body == "12345"
    email = Email(
        "s", "123456", EmailAddress("a@a.com"), EmailAddress("b@b.com")
    )
    email.add_short_body(5)
    assert email.short_body == "12345..."
...
def test_prepare_equivalence(subject, body, expected):
    email = Email(
        subject, body, EmailAddress("a@a.com"), EmailAddress("b@b.com")
    )
    email.prepare()
